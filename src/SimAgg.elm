...
calculateFragmentDistribution allelePair = 
    let
        ...
        relFreqWithTrip = 
            List.foldl (\size acc -> Array.set size (Maybe.withDefault 0 (Array.get size acc) + 10) acc) relFreq (alleleAFragmentSizes ++ alleleBFragmentSizes)
...
arrayValuesToPolylinePoints array = 
    let
        list = 
            array 
            |> Array.Extra.indexedMapToList (\i v -> (toFloat i, toFloat v * 1.005 ^ (0 - (toFloat i))))
    in 
    list 